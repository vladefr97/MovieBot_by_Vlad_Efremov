...
public class MovieBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasMessage() && update.getMessage().hasText()) {
            ...
            switch (receviedMsgText) {
                ...
                default:
                    if (!receviedMsgText.startsWith("/")) {
                        receviedMsgText = receviedMsgText.replace(' ', '+');
                        movies = MOVIE_FINDER.getMovies(receviedMsgText);
                        answerOnMovieRequest(update);
                    } else {
                        ...
                        if (array[0].toLowerCase().equals("/details")) {
                            ...
                            if (IDs.size() != 0) {
                                detailedMovies = MOVIE_FINDER.getMoviesDetailes(IDs);
    ...
    private void answerOnMovieRequest(Update update) {
    ...
    private void answerOnText(Update update, String answer) {
        long chatID = update.getMessage().getChatId();
